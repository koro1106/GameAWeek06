...
namespace ithappy.Animals_FREE
{
    ...
    public class MovePlayerInput : MonoBehaviour
    {
        [Header("Character")]
        [SerializeField]
        private string m_HorizontalAxis = "Horizontal"; // ADキーの入力ソース
        [SerializeField]
        private string m_VerticalAxis = "Vertical"; // 使用しない
        [SerializeField]
        private string m_JumpButton = "Jump";
        [SerializeField]
        private KeyCode m_RunKey = KeyCode.LeftShift;
        ...
        public void GatherInput()
        {
            // ADキーの入力 (Horizontal Axis) を取得し、それを垂直軸 (Y) に設定する。
            // Dキー (+1) で前進、Aキー (-1) で後退になる。
            // 水平軸の移動 (X) は 0 に設定する。
            m_Axis = new Vector2(0f, Input.GetAxis(m_HorizontalAxis));

            m_IsRun = Input.GetKey(m_RunKey);
            // スペースキー (デフォルトで Jump ボタン) の入力を取得
            m_IsJump = Input.GetButton(m_JumpButton);
