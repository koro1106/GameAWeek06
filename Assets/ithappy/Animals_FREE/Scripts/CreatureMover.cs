...
namespace ithappy.Animals_FREE
{
    ...
    public class CreatureMover : MonoBehaviour
    {
        ...
        private void OnValidate()
        {
            ...
            float jumpSpeed = Mathf.Sqrt(2f * m_JumpHeight * Physics.gravity.magnitude);
            m_Movement?.SetStats(m_WalkSpeed / 3.6f, m_RunSpeed / 3.6f, m_RotateSpeed, jumpSpeed, m_Space);
        }
        ...
        private void Awake()
        {
            ...
            // キャラクターの初期方向をワールドの右方向 (Vector3.right) に固定
            m_Transform.rotation = Quaternion.LookRotation(Vector3.right);

            float jumpSpeed = Mathf.Sqrt(2f * m_JumpHeight * Physics.gravity.magnitude);

            m_Movement = new MovementHandler(m_Controller, m_Transform, m_WalkSpeed / 3.6f, m_RunSpeed / 3.6f, m_RotateSpeed, jumpSpeed, m_Space);
            m_Animation = new AnimationHandler(m_Animator, m_VerticalID, m_StateID);
        }
        ...
        public void SetInput(in Vector2 axis, in Vector3 target, in bool isRun, in bool isJump)
        {
            ...
            // 移動入力の判定 (垂直軸 = A/D入力のみチェック)
            if (Mathf.Abs(m_Axis.y) < Mathf.Epsilon)
            {
                m_Axis = Vector2.zero;
                m_IsMoving = false;
            }
            ...
            {
                m_Axis = new Vector2(0f, Mathf.Clamp(m_Axis.y, -1f, 1f));
                m_IsMoving = true;
            }
        }
        ...
        private class MovementHandler
        {
            ...
            private float m_jumpTimer;
            // 回転ロジックが削除されたため、m_LastForwardは固定 (使わないが初期化)
            private Vector3 m_LastForward = Vector3.right;

            public MovementHandler(CharacterController controller, Transform transform, float walkSpeed, float runSpeed, float rotateSpeed, float jumpSpeed, Space space)
            {
                m_Controller = controller;
                m_Transform = transform;
            ...
            public void SetStats(float walkSpeed, float runSpeed, float rotateSpeed, float jumpSpeed, Space space)
            {
                m_WalkSpeed = walkSpeed;
                m_RunSpeed = runSpeed;
                m_JumpSpeed = jumpSpeed;
            ...
            public void Move(float deltaTime, in Vector2 axis, in Vector3 target, bool isRun, bool isMoving, bool isJump, out Vector2 animAxis, out bool isAir)
            {
                ...
                // 回転ロジックがないため、Turn/UpdateRotation は呼ばれない
                // m_LastForward の更新も不要
            ...
            private void ConvertMovement(in Vector2 axis, in Vector3 targetForward, out Vector3 movement)
            {
                ...
                // 移動方向をワールドの X 軸 (右) に固定
                forward = Vector3.right;
                right = Vector3.forward;

                // axis.y (A/D入力) のみを forward (Vector3.right) に適用
                // D (+1) -> 右へ移動, A (-1) -> 左へ移動
                movement = axis.x * right + axis.y * forward;
            ...
            private void CaculateGravity(float deltaTime, bool isJump, out bool isAir)
            {
                ...
                if (m_Controller.isGrounded)
                {
                    ...
                    if (isJump)
                    {
                        m_GravityAcelleration.y = m_JumpSpeed;
                        isAir = true;
                    }
                    return;
                }
            ...
            private void GenAnimationAxis(in Vector3 movement, out Vector2 animAxis)
            {
                // アニメーション軸は、キャラクターのローカル座標を基準に計算
                // キャラクターは Vector3.right を向いているため、
                // 前後移動は m_Transform.forward と一致する (Vector3.right)

                float forwardMove = Vector3.Dot(movement, m_Transform.forward);
                float rightMove = Vector3.Dot(movement, m_Transform.right);

                // 前後移動 (A/D) を Vertical (Y) 軸のアニメーション入力として渡す
                animAxis = new Vector2(rightMove, forwardMove);
            }
        }
        ...
        private class AnimationHandler
        {
            ...
            public void Animate(in Vector2 axis, float state, float deltaTime)
            {
                ...
                m_FlowAxis = Vector2.ClampMagnitude(m_FlowAxis + k_InputFlow * deltaTime * (axis - m_FlowAxis), 1f);
